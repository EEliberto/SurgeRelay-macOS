...
final class SurgeRelayTests: XCTestCase {
    ...
    func testMergerAddsSourceTogglesAndRemovesDeviceRestrictions() throws {
        ...
        XCTAssertTrue(merged.contains("#!name=Surge Relay (iOS)"))
        XCTAssertTrue(merged.contains("#!desc=由 Surge Relay 整合 2 个模块，iOS 专用。"))
    ...
    func testSearchQueryCleaning() {
        XCTAssertEqual(AppModel.cleanSearchQuery("拼多多去广告 (ZenmoFeiShi)"), "拼多多")
        XCTAssertEqual(AppModel.cleanSearchQuery("微博（去广告版）"), "微博")
        XCTAssertEqual(AppModel.cleanSearchQuery("YouTube净化"), "YouTube")
        XCTAssertEqual(AppModel.cleanSearchQuery("WeChat (Clean)"), "WeChat")
        XCTAssertEqual(AppModel.cleanSearchQuery("抖音 净化版"), "抖音 版")
    }
}
